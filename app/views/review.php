		...
		<section class="content">
			<div class="container-fluid">
				<div class="card shadow-sm">
					...
					<!-- ── Doctor voice notes ── -->
					<div class="card border-secondary mb-3" id="doctorVoiceCard">
						<div class="card-header py-2 bg-light">
							<span class="text-muted small text-uppercase font-weight-bold" style="letter-spacing:.05em;">
								<i class="fas fa-microphone-alt mr-1 text-secondary"></i>Doctor Voice Notes
							</span>
						</div>
						<div class="card-body py-2">
							<div class="d-flex align-items-center flex-wrap">
								<button type="button" data-vn-start data-vn-source="DOCTOR" class="btn btn-outline-danger btn-sm mr-2 mb-1"><i class="fas fa-microphone mr-1"></i>Record</button>
								<button type="button" data-vn-stop data-vn-source="DOCTOR" class="btn btn-outline-secondary btn-sm mr-2 mb-1" disabled><i class="fas fa-stop mr-1"></i>Stop &amp; Upload</button>
								<span data-vn-timer data-vn-source="DOCTOR" class="text-muted small mr-2">00:00</span>
								<span data-vn-status data-vn-source="DOCTOR" class="small"></span>
							</div>
							<small class="form-text text-muted">Dictate findings, plan, or a note-to-self. Microphone permission required. Max 25 MB per clip.</small>
							<div class="mt-2">
								<div class="text-muted small text-uppercase mb-1"><i class="fas fa-user mr-1"></i>Patient recordings (from intake)</div>
								<div data-vn-list data-vn-source="PATIENT"></div>
							</div>
							<div class="mt-2">
								<div class="text-muted small text-uppercase mb-1"><i class="fas fa-user-md mr-1"></i>Doctor recordings</div>
								<div data-vn-list data-vn-source="DOCTOR"></div>
							</div>
						</div>
					</div>
...
<!-- ── Voice notes recorder (patient + doctor playback) ─────────── -->
<script>
(function () {
	var csrfToken     = <?= json_encode($_SESSION['csrf_token']) ?>;
	var caseSheetId   = <?= (int)$csId ?>;
	var currentUserId = <?= (int)($_SESSION['user_id'] ?? 0) ?>;
	var canAdminDel   = <?= in_array($_SESSION['user_role'] ?? '', ['SUPER_ADMIN','ADMIN'], true) ? 'true' : 'false' ?>;
	...
	function renderNote(n) {
		var canDel = (n.recorded_by_user_id == currentUserId) || canAdminDel;
	...
	function loadList(source) {
		...
		$.getJSON('intake.php?action=list-voice-notes&case_sheet_id=' + caseSheetId + '&source=' + source, function (r) {
			$list.empty();
			if (!r.success || !r.notes.length) { $list.append('<small class="text-muted">No recordings yet.</small>'); return; }
			r.notes.forEach(function (n) { $list.append(renderNote(n)); });
		});
	}
	...
	function startRecorder(source) {
		...
		$start.on('click', function () {
			...
			navigator.mediaDevices.getUserMedia({ audio: true }).then(function (s) {
				stream = s; chunks = [];
				try { mediaRec = new MediaRecorder(s, { mimeType: 'audio/webm' }); }
				...
				mediaRec.onstop = function () {
					...
					clearInterval(tick); $timer.text('00:00');
					var fd = new FormData();
				...
				mediaRec.start(); startedAt = Date.now();
				tick = setInterval(function () { $timer.text(fmtSecs(Math.round((Date.now() - startedAt) / 1000))); }, 500);
				$start.prop('disabled', true); $stop.prop('disabled', false); $status.text('Recording…');
			}).catch(function () { $status.addClass('text-danger').text('Microphone permission denied.'); });
		});
		$stop.on('click', function () {
			if (mediaRec && mediaRec.state !== 'inactive') mediaRec.stop();
			$stop.prop('disabled', true); $start.prop('disabled', false);
		});
	}

	['PATIENT','DOCTOR'].forEach(function (src) {
		if (el(src, 'start')) startRecorder(src);
		if (el(src, 'list')) loadList(src);
	});
})();
</script>
